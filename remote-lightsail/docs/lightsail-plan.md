...
```powershell
Copy-Item remote-lightsail\terraform\terraform.tfvars.example remote-lightsail\terraform\terraform.tfvars
Copy-Item remote-lightsail\scripts\config.example.env remote-lightsail\scripts\config.env
...
3. `remote-lightsail/scripts/config.env`의 `ORCA_VERSION`을 변경한다.
